...
# FIX: Added ":generateContent" action — this was causing the 404 error
...
def generate_latex():
    ...
    # FIX: Rebuild GEMINI_API_URL at request time to pick up env var correctly
    api_url = f"{BASE_GEMINI_API_URL}/{GEMINI_MODEL}:generateContent?key={GEMINI_API_KEY}"

    # System instruction using the proper Gemini systemInstruction field
    system_instruction_text = (
        ...
        "Do not include any introductory text, explanations, markdown, or code fences like "
        "'Here is the code:' or '```latex'. "
        "Your response should be only the valid LaTeX code itself and nothing else."
    )

    try:
        # FIX: Use the proper systemInstruction field instead of manually prepending to prompt
        payload = {
            "systemInstruction": {
        ...
        try:
            if not response_data.get("candidates"):
                ...
                if prompt_feedback:
                    ...
                    return jsonify({
                        "error": "Prompt was blocked by API.",
        ...
        # Clean up markdown/code fences if model ignores instructions
        ...
        return jsonify({"latexCode": raw_latex})
        ...
        hint = ""
        if status_code == 404:
            hint = "The model name might be wrong or the API endpoint is incorrect. Call /api/list-models to verify."
...
def render_diagram():
    ...
    try:
        ...
        if not svg_image_data:
            return jsonify({
                "error": "Rendering service failed to return an image.",
